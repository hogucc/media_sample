...
module Builder
  def self.check_for_name_collision(klass, method_name, _defined_constant = nil)
    raise "Name Collision: Method '#{method_name}' is already defined in #{klass}" if klass.method_defined?(method_name.to_s)
  end
end

if !defined?(Builder::XChar) && !String.method_defined?(:encode)
...
if String.method_defined?(:encode)
  module Builder
    module XChar # :nodoc:
      CP1252_DIFFERENCES, UNICODE_EQUIVALENT = Builder::XChar::CP1252.each
                                                                     .inject([[], []]) do |(domain, range), (key, value)|
                                                 [domain << key, range << value]
                                               end.map { |seq| seq.pack('U*').force_encoding('utf-8') }
      ...
      INVALID_XML_CHAR = Regexp.new('[^' +
        Builder::XChar::VALID.map do |item|
          case item
          when Integer
  ...
  class String
    ...
    def to_xs(escape = true)
      ...
      unpack('C*').map(&:xchr).join # ISO-8859-1, WIN-1252
    end
  end
end
