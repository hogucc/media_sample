...
module Mail
  ...
  class Header
    ...
    def fields=(unfolded_fields)
      ...
      if unfolded_fields.length > self.class.maximum_amount
    ...
    def [](name)
      ...
      if selected.length > 1
        selected.map { |f| f }
      elsif !Utilities.blank?(selected)
        selected.first
      end
    end
    ...
    def []=(name, value)
      name = dasherize(name)
      raise ArgumentError, "Header names may not contain a colon: #{name.inspect}" if name.include?(':')

      fn = name.downcase
      selected = select_field_for(fn)

      if !Utilities.blank?(selected) && value.nil?
        fields.delete_if { |f| selected.include?(f) }

      # User wants to change the field
      elsif !Utilities.blank?(selected) && limited_field?(fn)
        selected.first.update(fn, value)

      # User wants to create the field
      else
        # Need to insert in correct order for trace fields
        fields << Field.new(name.to_s, value, charset)
      end
      if dasherize(fn) == 'content-type'
        # Update charset if specified in Content-Type
        params = begin
                   self[:content_type].parameters
    ...
    LIMITED_FIELDS = %w[ date from sender reply-to to cc bcc
                         ...
                         content-id content-disposition content-location].freeze
    ...
    def each(&block)
      return fields.each(&block) if block
