    ...
    def build_smtp_session
      Net::SMTP.new(settings[:address], settings[:port]).tap do |smtp|
        if settings[:tls] || settings[:ssl]
          smtp.enable_tls(ssl_context) if smtp.respond_to?(:enable_tls)
        elsif settings[:enable_starttls]
          smtp.enable_starttls(ssl_context) if smtp.respond_to?(:enable_starttls)
        elsif settings[:enable_starttls_auto]
          smtp.enable_starttls_auto(ssl_context) if smtp.respond_to?(:enable_starttls_auto)
        end
