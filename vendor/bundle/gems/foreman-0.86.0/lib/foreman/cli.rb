...
class Foreman::CLI < Foreman::Thor
  ...
  rescue Foreman::Export::Exception => e
    error e.message
  end
  ...
  def check
    check_procfile!
    engine.load_procfile(procfile)
    error 'no processes defined' if engine.processes.empty?
  ...
  def run(*args)
    ...
    engine.load_procfile(procfile) if File.file?(procfile)
  ...
  def procfile
    if options[:procfile] then options[:procfile]
    elsif options[:root] then File.expand_path(File.join(options[:root], 'Procfile'))
    else 'Procfile'
    end
  end
