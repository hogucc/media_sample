...
class MiniPortile
  ...
  def activate
    ...
    }.select { |_env, path| File.directory?(path) }
    ...
    vars.each do |var, path|
      ...
      ENV[var] = "#{full_path}#{File::PATH_SEPARATOR}#{old_value}" unless old_value.include?(full_path)
    end
    ...
    if File.exist?(lib_path) && (@host != @original_host)
      ...
      ENV['LDFLAGS'] = "-L#{full_path} #{old_value}".strip unless old_value.include?(full_path)
    end
  end
  ...
  def verify_file(file)
    if file.key?(:gpg)
      ...
      digest = if exp = file[:sha256] then Digest::SHA256
               elsif exp = file[:sha1] then Digest::SHA1
               elsif exp = file[:md5] then Digest::MD5
      end
      if digest
        is = digest.file(file[:local_path]).hexdigest
        raise "Downloaded file '#{file[:local_path]}' has wrong hash: expected: #{exp} is: #{is}" unless is == exp.downcase
      end
    end
  end
  ...
  def execute(action, command, options = {})
    ...
    Dir.chdir (options.fetch(:cd) { work_path }) do
      message "Running '#{action}' for #{@name} #{@version}... " if options.fetch(:initial_message) { true }
      ...
      if Process.respond_to?(:spawn) && !RbConfig.respond_to?(:java)
        ...
        redirected = if command.is_a?(Array)
                       %(#{command.map(&:shellescape).join(' ')} > #{log_out.shellescape} 2>&1)
                     else
                       %(#{command} > #{log_out.shellescape} 2>&1)
                     end
        system redirected
      end
  ...
  def download_file_http(url, full_path, count = 3)
    ...
    with_tempfile(filename, full_path) do |temp_file|
      ...
      params = {
        ...
        :content_length_proc => ->(length) { total = length },
        :progress_proc => lambda { |bytes|
          if total
            new_progress = (bytes * 100) / total
            message format("\rDownloading %s (%3d%%) ", filename, new_progress)
            progress = new_progress
          else
            # Content-Length is unavailable because Transfer-Encoding is chunked
            message format("\rDownloading %s ", filename)
          end
        }
      }
      ...
      begin
        ...
        count -= 1
        return download_file(e.url, full_path, count - 1)
