...
class TestProxy < TestCase
  ...
  def assert_proxy_auth(_expected, request)
    if request =~ /^Proxy-Authorization: Basic (.*)/
      assert_equal 'user: @name:@12: üMp', Regexp.last_match(1).unpack1('m').force_encoding(__ENCODING__)
    else
  ...
  def test_http_proxy
    ...
    request = with_dummy_proxy do |url, _thread|
      ENV['http_proxy'] = url
      begin
        recipe.download
    ...
    assert_match(%r{GET http://myserver/path/to/tar.gz}, request)
  end

  def test_http_proxy_with_basic_auth
    recipe = MiniPortile.new('test http_proxy', '1.0.0')
    recipe.files << 'http://myserver/path/to/tar.gz'
    request = with_dummy_proxy('user: @name', '@12: üMp') do |url, _thread|
      ENV['http_proxy'] = url
      begin
        recipe.download
      rescue StandardError
  ...
  def test_ftp_proxy
    ...
    assert_match(%r{GET ftp://myserver/path/to/tar.gz}, request)
  end
