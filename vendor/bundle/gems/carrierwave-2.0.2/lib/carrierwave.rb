...
if defined?(Merb)
  ...
  module CarrierWave
    class Railtie < Rails::Railtie
      initializer 'carrierwave.setup_paths' do |app|
        ...
        I18n.load_path.prepend(File.join(File.dirname(__FILE__), 'carrierwave', 'locale', 'en.yml')) if available_locales.blank? || available_locales.include?(:en)
      end
  ...
  CarrierWave.root = if defined?(Padrino) && defined?(PADRINO_ROOT)
                       File.join(PADRINO_ROOT, 'public')
                     else
