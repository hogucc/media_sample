...
module Addressable
  ...
  class URI
    ...
    class << self
      alias encode_component encode_component
    end
    ...
    def self.form_unencode(encoded_value)
      ...
      split_values.map do |(key, value)|
        [
          key ? unencode_component(
            key.gsub('+', '%20')
          ).gsub(/(\r\n|\n|\r)/, "\n") : nil,
          value ? unencode_component(
            value.gsub('+', '%20')
          ).gsub(/(\r\n|\n|\r)/, "\n") : nil
        ]
      end
    ...
    def scheme=(new_scheme)
      ...
      raise InvalidURIError, "Invalid scheme format: #{new_scheme}" if new_scheme && new_scheme !~ /\A[a-z][a-z0-9\.\+\-]*\z/i
    ...
    def normalized_user
      ...
      @normalized_user ||= begin
        if normalized_scheme =~ /https?/ && user.strip.empty? &&
    ...
    def normalized_password
      ...
      @normalized_password ||= begin
        if normalized_scheme =~ /https?/ && password.strip.empty? &&
    ...
    def password=(new_password)
      raise TypeError, "Can't convert #{new_password.class} into String." if new_password && !new_password.respond_to?(:to_str)
    ...
    def userinfo=(new_userinfo)
      raise TypeError, "Can't convert #{new_userinfo.class} into String." if new_userinfo && !new_userinfo.respond_to?(:to_str)
    ...
    def normalized_port
      ...
      @normalized_port ||= begin
        if URI.port_mapping[normalized_scheme] == port
          nil
        else
          port
        end
    ...
    def port=(new_port)
      ...
      raise InvalidURIError, 'Invalid encoding in port' if new_port.respond_to?(:valid_encoding?) && !new_port.valid_encoding?
    ...
    def fragment=(new_fragment)
      raise TypeError, "Can't convert #{new_fragment.class} into String." if new_fragment && !new_fragment.respond_to?(:to_str)
    ...
    def merge(hash)
      ...
      uri.defer_validation do
        ...
        # like userinfo and authority.
        uri.scheme =
          hash.key?(:scheme) ? hash[:scheme] : scheme
        if hash.key?(:authority)
          uri.authority =
            hash.key?(:authority) ? hash[:authority] : authority
        end
        if hash.key?(:userinfo)
          uri.userinfo =
            hash.key?(:userinfo) ? hash[:userinfo] : userinfo
        end
        if !hash.key?(:userinfo) && !hash.key?(:authority)
          uri.user =
            hash.key?(:user) ? hash[:user] : user
          uri.password =
            hash.key?(:password) ? hash[:password] : password
        end
        unless hash.key?(:authority)
          uri.host =
            hash.key?(:host) ? hash[:host] : host
          uri.port =
            hash.key?(:port) ? hash[:port] : port
        end
        uri.path =
          hash.key?(:path) ? hash[:path] : path
        uri.query =
          hash.key?(:query) ? hash[:query] : query
        uri.fragment =
          hash.key?(:fragment) ? hash[:fragment] : fragment
      end
      ...
      if normalized_self.scheme == uri.scheme
        ...
        if normalized_self.authority == uri.authority
          ...
          if normalized_self.path == uri.path
            ...
            if (uri.path != SLASH) && components[:path]
              ...
              while !self_splitted_path.empty? && !uri_splitted_path.empty? && (self_dir == uri_dir)
                self_dir = self_splitted_path.shift
                uri_dir = uri_splitted_path.shift
              end
    ...
    #   otherwise.
    def ==(uri)
      return false unless uri.is_a?(URI)

      normalize.to_s == uri.normalize.to_s
    end
    ...
    def replace_self(uri)
      ...
      instance_variables.each do |var|
        remove_instance_variable(var) if instance_variable_defined?(var) && var != :@validation_deferred
      end
