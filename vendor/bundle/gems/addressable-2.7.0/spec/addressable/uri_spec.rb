...
describe Addressable::URI, 'when parsed from ' \
  ...
  it 'should raise an error if routing is attempted' do
    ...
    end).to raise_error(ArgumentError, %r{relative/path/to/resource})
    expect(lambda do
...
describe Addressable::URI, 'when parsed from ' \
  ...
  it 'should raise an error if routing is attempted' do
    expect(lambda do
      @uri.route_to('http://example.com/')
    end).to raise_error(ArgumentError, %r{//example.com/})
    expect(lambda do
      @uri.route_from('http://example.com/')
    end).to raise_error(ArgumentError, %r{//example.com/})
  end
...
describe Addressable::URI, 'when parsed from ' \
  ...
  it 'should have correct array query values' do
    expect(@uri.query_values(Array)).to eq(
      [%w[one two], %w[one three], %w[one four]]
    )
  end
...
describe Addressable::URI, 'when parsed from ' \
  ...
  it 'should properly force the encoding' do
    ...
    expect(display_string.encoding.to_s).to eq(Encoding::UTF_8.to_s) if display_string.respond_to?(:encoding)
  end
...
describe Addressable::URI, 'when assigning query values' do
  ...
  it "should correctly assign {:a => 'a', :b => ['c', 'd', 'e']}" do
    @uri.query_values = { a: 'a', b: %w[c d e] }
  ...
  it 'should correctly assign ' \
    ...
    @uri.query_values = [%w[b c], %w[b a], %w[a a]]
