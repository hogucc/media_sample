...
describe Rack::Mount do
  let(:app_one) do
    proc { |_env| [200, { 'Content-Type' => 'text/plain' }, 'Route 1'] }
  end
  let(:app_two) do
    proc { |_env| [200, { 'Content-Type' => 'text/plain' }, 'Route 2'] }
  end
  let(:app_three) do
    proc { |_env| [200, { 'Content-Type' => 'text/plain' }, 'Route 3'] }
  end
  let(:routes) do
  ...
  it 'should generate from routes with Addressable::Template' do
    path, = routes.generate(:path_info, :two, id: '654')
    expect(path).to eq '/two/'
  end

  it 'should generate from routes with Addressable::Template using defaults' do
    path, = routes.generate(:path_info, :two, {})
    expect(path).to eq '/two/'
  end
