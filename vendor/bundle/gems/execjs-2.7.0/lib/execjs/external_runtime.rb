  ...
  class ExternalRuntime < Runtime
    class Context < Runtime::Context
      ...
      def exec(source, _options = {})
        ...
        filepath = if ExecJS.cygwin?
                     `cygpath -m #{tmpfile.path}`.rstrip
                   else
                     tmpfile.path
                   end
      ...
      def create_tempfile(basename)
        ...
        Dir::Tmpname.create(basename) do |tmpname|
          ...
          tmpfile = File.open(tmpname, mode, 0o600)
        end
        tmpfile
      end

      def write_to_tempfile(contents)
        tmpfile = create_tempfile(%w[execjs js])
    ...
    def initialize(options)
      ...
      instance_eval generate_compile_method(@runner_path) if @runner_path
    end
    ...
    def locate_executable(command)
      ...
      if ExecJS.windows? && File.extname(command) == ''
        ENV['PATHEXT'].split(File::PATH_SEPARATOR).each do |p|
          commands << (command + p)
        end
      end
    ...
    if ExecJS.windows?
      def exec_runtime(filename)
        path = Dir::Tmpname.create(%w[execjs json]) {}
        begin
          command = binary.split(' ') << filename
          `#{shell_escape(*command)} 2>&1 > #{path}`
          output = File.open(path, 'rb', @popen_options, &:read)
        ensure
