...
module ActionView
  class DependencyTracker # :nodoc:
    ...
    class ERBTracker # :nodoc:
      EXPLICIT_DEPENDENCY = /# Template Dependency: (\S+)/.freeze
      ...
      def add_dynamic_dependency(dependencies, dependency)
        dependencies << "#{dependency.pluralize}/#{dependency.singularize}" if dependency
      end

      def add_static_dependency(dependencies, dependency)
        if dependency
          dependencies << if dependency.include?('/')
                            dependency
                          else
      ...
      def resolve_directories(wildcard_dependencies)
        ...
        wildcard_dependencies.flat_map do |query, _templates|
