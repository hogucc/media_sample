...
module ActionView
  ...
  class Resolver
    ...
    def _find_all(name, prefix, partial, details, _key, locals)
    ...
    # normalized.
    def find_templates(_name, _prefix, _partial, _details, _locals = [])
    ...
    def cached(key, path_info, _details, locals)
  ...
  class PathResolver < Resolver #:nodoc:
    EXTENSIONS = { locale: '.', formats: '.', variants: '+', handlers: '.' }.freeze
    ...
    def query(path, details, _formats, locals, cache:)
    ...
    def reject_files_external_to_app(files)
      files.select { |filename| inside_path?(@path, filename) }
    end
    ...
    def extract_handler_and_format_and_variant(path)
      ...
      format = if format
                 ...
                 handler.default_format if handler.respond_to?(:default_format) # default_format can return nil
      end
