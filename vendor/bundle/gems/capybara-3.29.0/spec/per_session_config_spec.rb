...
RSpec.describe Capybara::SessionConfig do
  describe 'threadsafe' do
    it 'defaults to global session options' do
      ...
      [:default_host, :app_host, :always_include_port, :run_server, :default_selector, :default_max_wait_time, :ignore_hidden_elements, :automatic_reload, :match, :exact, :raise_server_errors, :visible_text_only, :automatic_label_click, :enable_aria_label, :save_path, :asset_host].each do |m|
