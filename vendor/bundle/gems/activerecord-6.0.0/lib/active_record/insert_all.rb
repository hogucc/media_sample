...
module ActiveRecord
  class InsertAll # :nodoc:
    ...
    def initialize(model, inserts, on_duplicate:, returning: nil, unique_by: nil)
      ...
      @inserts = inserts
      @keys = inserts.first.keys.map(&:to_s).to_set
    ...
    def verify_attributes(attributes)
      raise ArgumentError, 'All objects being inserted must have the same keys' if keys != attributes.keys.to_set
    end
    ...
    class Builder # :nodoc:
      ...
      def initialize(insert_all)
        ...
        @model = insert_all.model
        @connection = insert_all.connection
      end
