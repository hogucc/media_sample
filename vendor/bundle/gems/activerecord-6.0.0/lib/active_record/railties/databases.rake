...
db_namespace = namespace :db do
  ...
  namespace :migrate do
    ...
    task redo: :load_config do
      raise 'Empty VERSION provided' if ENV['VERSION']&.empty?
  ...
  task prepare: :load_config do
    ...
    ActiveRecord::Base.configurations.configs_for(env_name: ActiveRecord::Tasks::DatabaseTasks.env).each do |db_config|
      ...
      ActiveRecord::Tasks::DatabaseTasks.migrate
      ActiveRecord::Tasks::DatabaseTasks.dump_schema(db_config.config, ActiveRecord::Base.schema_format, db_config.spec_name) if ActiveRecord::Base.dump_schema_after_migration
  ...
  namespace :fixtures do
    ...
    task identify: :load_config do
      ...
      puts %(The fixture ID for "#{label}" is #{ActiveRecord::FixtureSet.identify(label)}.) if label

      base_dir = ActiveRecord::Tasks::DatabaseTasks.fixtures_path

      Dir["#{base_dir}/**/*.yml"].each do |file|
        next unless data = YAML.safe_load(ERB.new(IO.read(file)).result)

        data.each_key do |key|
          key_id = ActiveRecord::FixtureSet.identify(key)

          puts "#{file}: #{key} (#{key_id})" if key == label || key_id == id.to_i
        end
      end
    end
  end
  ...
  namespace :test do
    ...
    task load_schema: %w[db:test:purge] do
      ...
      ActiveRecord::Base.establish_connection(ActiveRecord::Base.configurations.default_hash(ActiveRecord::Tasks::DatabaseTasks.env)) if should_reconnect
    end
    ...
    task prepare: :load_config do
      db_namespace['test:load'].invoke unless ActiveRecord::Base.configurations.blank?
    end
  end
end
  ...
  namespace :install do
    ...
    task migrations: :'db:load_config' do
      ...
      Rails.application.migration_railties.each do |railtie|
        ...
        railties[railtie.railtie_name] = railtie.root + ENV['MIGRATIONS_PATH'] unless ENV['MIGRATIONS_PATH'].blank?
      end

      on_skip = proc do |name, migration|
        puts "NOTE: Migration #{migration.basename} from #{name} has been skipped. Migration with the same name already exists."
      end

      on_copy = proc do |name, migration|
