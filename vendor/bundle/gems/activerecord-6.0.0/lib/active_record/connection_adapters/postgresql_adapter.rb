  ...
  module ConnectionAdapters
    ...
    class PostgreSQLAdapter < AbstractAdapter
      ...
      def supports_pgcrypto_uuid?
        database_version >= 90_400
      end

      def supports_optimizer_hints?
        @has_pg_hint_plan = extension_available?('pg_hint_plan') unless defined?(@has_pg_hint_plan)
        @has_pg_hint_plan
      end
      ...
      def get_advisory_lock(lock_id) # :nodoc:
        raise(ArgumentError, 'PostgreSQL requires advisory lock ids to be a signed 64 bit integer') unless lock_id.is_a?(Integer) && lock_id.bit_length <= 63
      ...
      def enable_extension(name)
        exec_query("CREATE EXTENSION IF NOT EXISTS \"#{name}\"").tap do
          reload_type_map
        end
      end

      def disable_extension(name)
        exec_query("DROP EXTENSION IF EXISTS \"#{name}\" CASCADE").tap do
          reload_type_map
        end
      end
      ...
      def check_version # :nodoc:
        raise "Your version of PostgreSQL (#{database_version}) is too old. Active Record supports PostgreSQL >= 9.3." if database_version < 90_300
      end
      ...
      def extract_value_from_default(default)
        ...
        when /\A[\(B]?'(.*)'.*::"?([\w. ]+)"?(?:\[\])?\z/m
          ...
          if Regexp.last_match(1) == 'now' && Regexp.last_match(2) == 'date'
            nil
          else
      ...
      def has_default_function?(default_value, default)
        !default_value && /\w+\(.*\)|\(.*\)::\w+|CURRENT_DATE|CURRENT_TIMESTAMP/.match?(default)
      end
      ...
      def load_additional_types(oids = nil)
        ...
        query += if oids
                   'WHERE t.oid::integer IN (%s)' % oids.join(', ')
                 else
