      ...
      module SchemaStatements # :nodoc:
        ...
        def internal_string_options_for_primary_key
          super.tap do |options|
            options[:collation] = collation.sub(/\A[^_]+/, 'utf8') if !row_format_dynamic_by_default? && CHARSETS_OF_4BYTES_MAXLEN.include?(charset)
          end
        end
        ...
        CHARSETS_OF_4BYTES_MAXLEN = %w[utf8mb4 utf16 utf16le utf32].freeze

        def row_format_dynamic_by_default?
          database_version >= if mariadb?
                                '10.2.2'
                              else
                                '5.7.9'
                              end
        ...
        def default_row_format
          ...
          unless defined?(@default_row_format)
            @default_row_format = ('ROW_FORMAT=DYNAMIC' if query_value("SELECT @@innodb_file_per_table = 1 AND @@innodb_file_format = 'Barracuda'") == 1)
          end
