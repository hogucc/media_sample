  ...
  class Migration
    ...
    class CommandRecorder
      ReversibleAndIrreversibleMethods = [:create_table, :create_join_table, :rename_table, :add_column, :remove_column,
                                          ...
                                          :change_column_comment, :change_table_comment].freeze
      include JoinTable
      ...
      def record(*command, &block)
        @commands << if @reverting
                       inverse_of(*command, &block)
                     else
                       (command << block)
                     end
      end
      ...
      def invert_drop_table(args, &block)
        raise ActiveRecord::IrreversibleMigration, 'To avoid mistakes, drop_table is only reversible if given options or a block (can be empty).' if args.size == 1 && block.nil?
      ...
      def invert_add_index(args)
        ...
        options_hash[:column] = columns unless options_hash[:name]
      ...
      def invert_remove_index(args)
        ...
        if (options = options_or_column).is_a?(Hash)
          raise ActiveRecord::IrreversibleMigration, 'remove_index is only reversible if given a :column option.' unless options[:column]
      ...
      def invert_change_column_default(args)
        ...
        unless options&.is_a?(Hash) && options&.key?(:from) && options&.key?(:to)
      ...
      def invert_add_foreign_key(args)
        ...
        options = if add_options[:name]
                    { name: add_options[:name] }
                  elsif add_options[:column]
                    { column: add_options[:column] }
                  else
                    to_table
                  end
