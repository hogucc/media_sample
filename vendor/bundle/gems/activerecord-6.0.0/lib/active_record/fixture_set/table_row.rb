    ...
    class TableRow # :nodoc:
      ...
      def reflection_class
        @reflection_class ||= if @row.include?(model_metadata.inheritance_column_name)
                                begin
                                    @row[model_metadata.inheritance_column_name].constantize
      ...
      def resolve_enums
        model_class.defined_enums.each do |name, values|
          @row[name] = values.fetch(@row[name], @row[name]) if @row.include?(name)
        end
      end
      ...
      def resolve_sti_reflections
        ...
        reflection_class._reflections.each_value do |association|
          ...
          when :belongs_to
            ...
            if association.name.to_s != fk_name && value = @row.delete(association.name.to_s)
              if association.polymorphic? && value.sub!(/\s*\(([^\)]*)\)\s*$/, '')
                ...
                @row[association.foreign_type] = Regexp.last_match(1)
              end
          ...
          when :has_many
            add_join_records(HasManyThroughProxy.new(association)) if association.options[:through]
          end
        end
      end
