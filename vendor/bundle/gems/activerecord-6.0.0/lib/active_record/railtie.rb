      ...
      namespace :db do
        task :load_config do
          ...
          if defined?(ENGINE_ROOT) && engine = Rails::Engine.find(ENGINE_ROOT)
            ActiveRecord::Tasks::DatabaseTasks.migrations_paths += engine.paths['db/migrate'].to_a if engine.paths['db/migrate'].existent
          end
        end
      end
        ...
        config.after_initialize do |app|
          ActiveSupport.on_load(:active_record) do
            ...
            if File.file?(filename)
              ...
              cache = YAML.safe_load(File.read(filename))
