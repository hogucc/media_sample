...
module ActiveRecord::Associations::Builder # :nodoc:
  class BelongsTo < SingularAssociation #:nodoc:
    ...
    def self.add_counter_cache_callbacks(model, reflection)
      ...
      model.after_update lambda { |_record|
      ...
      klass.attr_readonly cache_column if klass&.respond_to?(:attr_readonly)
    end

    def self.touch_record(o, changes, foreign_key, name, touch, touch_method) # :nodoc:
      old_foreign_id = changes[foreign_key]&.first
      ...
      if old_foreign_id
        ...
        if reflection.polymorphic?
          ...
          klass = changes[foreign_type]&.first || o.public_send(foreign_type)
    ...
    def self.add_touch_callbacks(model, reflection)
      ...
      if reflection.counter_cache_column
        touch_callback = callback.call(:saved_changes)
    ...
    def self.add_destroy_callbacks(model, reflection)
      model.after_destroy ->(o) { o.association(reflection.name).handle_dependency }
    end
