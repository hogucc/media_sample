  ...
  module Associations
    ...
    module ThroughAssociation #:nodoc:
      ...
      def construct_join_attributes(*records)
        ...
        join_attributes = if association_primary_key == reflection.klass.primary_key && !options[:source_type]
                            { source_reflection.name => records }
                          else
                            {
        ...
        join_attributes[source_reflection.foreign_type] = [options[:source_type]] if options[:source_type]
      ...
      def stale_state
        owner[through_reflection.foreign_key]&.to_s if through_reflection.belongs_to?
      end
      ...
      def build_record(attributes)
        ...
        attributes[inverse.foreign_key] = target.id if inverse && target && !target.is_a?(Array)
