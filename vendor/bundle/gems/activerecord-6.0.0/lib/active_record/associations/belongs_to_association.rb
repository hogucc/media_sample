  ...
  module Associations
    ...
    class BelongsToAssociation < SingularAssociation #:nodoc:
      ...
      def decrement_counters_before_last_save
        model_was = if reflection.polymorphic?
                      owner.attribute_before_last_save(reflection.foreign_type).try(:constantize)
                    else
                      klass
                    end
        ...
        update_counters_via_scope(model_was, foreign_key_was, -1) if foreign_key_was && model_was < ActiveRecord::Base
      end
